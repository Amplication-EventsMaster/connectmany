...
export class CustomerResolverBase {
  ...
  async findFavoriteColors(
    ...
    const results = await this.service.findFavoriteColors(parent.id, args);
  ...
  @graphql.ResolveField(() => [Color], { name: "likedColors" })
  async findLikedColors(
    @graphql.Parent() parent: Customer,
    @graphql.Args() args: ColorFindManyArgs
  ): Promise<Color[]> {
    const results = await this.service.findLikedColors(parent.id, args);
